...
| `cargo-mcp.test.timeoutSecs` | `30` | Default wall-clock timeout in seconds for `cargo_test` runs. The budget covers only test execution — the clock starts when compilation and linking finish, so a slow build never trips the timeout. When Copilot calls `cargo_test` without an explicit `timeout_secs`, this value is used. Set to `0` to disable the default timeout. |
