...
| `cargo-mcp.test.timeoutSecs` | `30` | Default wall-clock timeout in seconds for `cargo_test` runs. Covers only test execution — the clock starts when compilation and linking finish, so a slow build never trips it. Routed by the server to whichever knob matches the call: in **unfiltered** mode it defaults `timeout_secs` (overall wall-clock); in **`test_filter` mode** it defaults `per_test_timeout_secs` (per-test idle watchdog that resets on every libtest boundary), so hung-test protection is on by default in filter mode while a long matched run can still complete. Explicit `timeout_secs` / `per_test_timeout_secs` arguments always override this default. Set to `0` to disable the server-provided default. In `test_filter` mode, `0` (or an absent setting) still leaves a hard-coded **30-second** per-test fallback in place so hung-test protection stays on; the only way to fully disable per-test protection for a call is an explicit `per_test_timeout_secs: 0` in the tool arguments. |
